...
neo_fft3d::engine::CoverPlaneGeometry FFT3DEngine::cover_geometry() const {
...
neo_fft3d::engine::FrameViewAdapter FFT3DEngine::frame_views() const {
...
neo_fft3d::FloatPlaneView FFT3DEngine::kernel_float_view(AlignedVector<float>& data) const {
...
neo_fft3d::ConstFloatPlaneView FFT3DEngine::kernel_float_view(const AlignedVector<float>& data) const {
  return neo_fft3d::make_float_plane_view(
    data.data(),
    outpitch,
    ep->bh,
...
neo_fft3d::ComplexBlockView FFT3DEngine::kernel_complex_blocks(AlignedVector<std::complex<float>>& data, int block_count) const {
  return neo_fft3d::make_complex_block_view(data.data(), outpitch, ep->bh, block_count);
}

neo_fft3d::ComplexBlockView FFT3DEngine::kernel_complex_blocks(std::complex<float>* data, int block_count) const {
  return neo_fft3d::make_complex_block_view(data, outpitch, ep->bh, block_count);
}

void FFT3DEngine::validate_backend() const {
  if (!fft_backend) {
...
void FFT3DEngine::normalize_params_for_sample_format() {
  float factor;
...
void FFT3DEngine::validate_overlap_params_and_apply_defaults() {
  if (ep->ow * 2 > ep->bw) {
...
void FFT3DEngine::validate_temporal_params() const {
  if (ep->bt < -1 || ep->bt > 5) {
    throw("bt must be -1 (Sharpen), 0 (Kalman), 1..5 (Wiener)");
  }
}

void FFT3DEngine::configure_geometry() {
...
void FFT3DEngine::configure_storage() {
  if (ep->bt == 0) // Kalman
  {
...
void FFT3DEngine::create_main_plans(const neo_fft3d::fft::PlanOptions& plan_options, const neo_fft3d::fft::PlanBuffers& plan_buffers) {
...
void FFT3DEngine::resize_window_buffers() {
...
void FFT3DEngine::initialize_overlap_windows() {
...
void FFT3DEngine::initialize_sharpen_window() {
...
void FFT3DEngine::initialize_dehalo_window() {
...
void FFT3DEngine::initialize_filter_windows() {
  initialize_overlap_windows();
  initialize_sharpen_window();
  initialize_dehalo_window();
}

void FFT3DEngine::initialize_normalization_params() {
...
void FFT3DEngine::initialize_kalman_buffers() {
  if (ep->bt == 0) // Kalman
  {
...
void FFT3DEngine::initialize_pattern_buffers() {
...
void FFT3DEngine::create_grid_plan(const neo_fft3d::fft::PlanOptions& plan_options, const neo_fft3d::fft::PlanBuffers& plan_buffers) {
  neo_fft3d::fft::GlobalLockGuard fftw_lock;
...
void FFT3DEngine::initialize_grid_sample(float* in, const neo_fft3d::fft::PlanOptions& plan_options,
...
void FFT3DEngine::ensure_fft_cache_capacity(std::size_t workspace_slot_count) {
  if (!fftcache) {
    return;
  }
...
void FFT3DEngine::transform_source_plane_to_spectrum(
...
void FFT3DEngine::write_spectrum_to_destination(
  std::complex<float>* spectrum,
  float* in,
  byte* coverbuf,
  const neo_fft3d::engine::FrameViewAdapter& views,
...
void FFT3DEngine::show_pattern_frame(
  int n,
  int pxf,
  int pyf,
...
bool FFT3DEngine::handle_pattern_frame(
  int n,
...
int FFT3DEngine::current_bt_for_frame(int n) const {
...
SharedFunctionParams FFT3DEngine::make_shared_function_params(int btcur) {
...
void FFT3DEngine::ensure_3d_pattern_initialized(int btcur) {
...
void FFT3DEngine::process_wiener_3d_frame(
  int n,
  int btcur,
  ds::VideoFrameProvider& provider,
  const neo_fft3d::engine::FrameViewAdapter& views,
...
void FFT3DEngine::process_wiener_frame(
  int n,
  int btcur,
  ds::VideoFrameProvider& provider,
  const ds::PlaneView& src_plane,
...
void FFT3DEngine::process_kalman_frame(
  int n,
  const ds::PlaneView& src_plane,
  const ds::MutablePlaneView& dst_plane,
  const neo_fft3d::engine::FrameViewAdapter& views,
...
void FFT3DEngine::process_sharpen_frame(
  const ds::PlaneView& src_plane,
  const ds::MutablePlaneView& dst_plane,
  const neo_fft3d::engine::FrameViewAdapter& views,
  byte* coverbuf,
...
FFT3DEngine::FFT3DEngine(
  EngineParams _ep,
  int _plane,
  std::shared_ptr<neo_fft3d::fft::FFTBackend> _fft_backend,
  std::unique_ptr<neo_fft3d::engine::FilterBackend> _backend
) :
  ep(std::make_unique<EngineParams>(_ep)),
  iop(std::make_unique<IOParams>()),
  plane(_plane),
  fft_backend(std::move(_fft_backend)),
...
void FFT3DEngine::ProcessFrame(int n, ds::VideoFrameProvider& provider, ds::MutableVideoFrameView dst) {
...
bool FFT3DEngine::CacheRefresh(int n) {
  if (!fftcache) {
    return false;
  }
