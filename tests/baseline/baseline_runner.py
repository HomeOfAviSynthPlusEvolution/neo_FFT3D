...
def render_vs_script(plugin_path: str, source: dict, params: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            *source_lines,
            f"clip = core.neo_fft3d.FFT3D(src{param_text})",
            "clip.set_output()",
            "",
        ]
    )
...
def render_avs_script(plugin_path: str, source: dict, params: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            *source_lines,
            f"return neo_fft3d(src{param_text})",
            "",
        ]
    )
...
def run_vs_case(case: dict, plugin_path: Path) -> list[dict]:
    ...
    clip = core.neo_fft3d.FFT3D(clip, **host_params(case, "vs"))
...
def run_avs_case(case: dict, plugin_path: Path, avs_dump: Path) -> list[dict]:
    ...
    results = []
    with tempfile.TemporaryDirectory(prefix="neo-fft3d-baseline-") as temp_dir_name:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate or verify neo-fft3d frame hash baselines.")
