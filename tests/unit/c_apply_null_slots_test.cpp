...
SharedFunctionParams make_params(
  ...
  sfp.pfactor = 0.0f;
  sfp.pattern2d = neo_fft3d::make_float_plane_view(pattern2d, sfp.outpitch, sfp.bh, sfp.outpitch * sizeof(float));
  sfp.pattern3d = neo_fft3d::make_float_plane_view(pattern3d, sfp.outpitch, sfp.bh, sfp.outpitch * sizeof(float));
  sfp.beta = 2.0f;
  sfp.degrid = 0.0f;
  sfp.gridsample = neo_fft3d::make_complex_block_view(gridsample, sfp.outpitch, sfp.bh, sfp.howmanyblocks);
  sfp.sharpen = 0.0f;
  sfp.sigmaSquaredSharpenMinNormed = 0.0f;
  sfp.sigmaSquaredSharpenMaxNormed = 0.0f;
  sfp.wsharpen = neo_fft3d::make_float_plane_view(wsharpen, sfp.outpitch, sfp.bh, sfp.outpitch * sizeof(float));
  sfp.dehalo = 0.0f;
  sfp.wdehalo = neo_fft3d::make_float_plane_view(wdehalo, sfp.outpitch, sfp.bh, sfp.outpitch * sizeof(float));
  sfp.ht2n = 0.0f;
  sfp.covar = neo_fft3d::make_complex_block_view(covar, sfp.outpitch, sfp.bh, sfp.howmanyblocks);
  sfp.covarProcess = neo_fft3d::make_complex_block_view(covar_process, sfp.outpitch, sfp.bh, sfp.howmanyblocks);
